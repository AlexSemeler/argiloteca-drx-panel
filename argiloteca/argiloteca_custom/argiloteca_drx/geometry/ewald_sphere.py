...
def ewald_radius(lambda_A: float) -> float:
    """
    Executa a etapa `ewald_radius` do módulo.

        Args:
            lambda_A:
                Parâmetro utilizado pela etapa `ewald_radius`.
        Returns:
    ...
    """
    return 1.0 / float(lambda_A)


def ewald_intersection_possible(lambda_A: float, d_A: float) -> bool:
    """
    Executa a etapa `ewald_intersection_possible` do módulo.

        Args:
            lambda_A:
                Parâmetro utilizado pela etapa `ewald_intersection_possible`.
            d_A:
                Parâmetro utilizado pela etapa `ewald_intersection_possible`.
        Returns:
            Resultado documentado pelo contrato do chamador.
        ...
        Notes:
            ...
            tratado como confirmação mineralógica completa.
    """
    return 1.0 / float(d_A) <= 2.0 / float(lambda_A)
