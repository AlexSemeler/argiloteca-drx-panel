...
class DrxV3EngineTest(unittest.TestCase):
    ...
    def test_panel_prefers_backend_ngc_grouping_with_legacy_fallback(self):
        ...
        self.assertIn("function buildNgcSummaryFromBackendGrouping", source)
        self.assertIn("function backendSummaryPreparationCounts", source)
        self.assertIn("function buildNgcInterpretationsFromBackend", source)
        self.assertIn("function backendWorkflowGroupsForSelection", source)
        self.assertIn("function backendInterpretationCandidateText", source)
        self.assertIn("function backendInterpretationEvidenceText", source)
        self.assertIn("ngc_evidence_summary", source)
        ...
        self.assertIn("const backendRows = buildNgcInterpretationsFromBackend(items)", source)
        self.assertIn("if (backendRows) return backendRows", source)
        self.assertIn("const backendSummary = buildNgcSummaryFromBackendGrouping(items, interpretations)", source)
        self.assertIn("if (backendSummary) return backendSummary", source)
